...
class HermesKanbanAdapter:
    """Reads, writes, syncs tasks, and logs agent activity events with local Hermes SQLite kanban database."""
    ...
    def list_tasks(self, limit: int = 50, status: str | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if status:
                rows = cursor.execute(
                    "SELECT id, title, body, status, assignee, priority, created_at, updated_at, result FROM tasks WHERE LOWER(status) = LOWER(?) ORDER BY rowid DESC LIMIT ?",
                    (status, limit),
                ).fetchall()
            else:
                rows = cursor.execute(
                    "SELECT id, title, body, status, assignee, priority, created_at, updated_at, result FROM tasks ORDER BY rowid DESC LIMIT ?",
                    (limit,),
                ).fetchall()
